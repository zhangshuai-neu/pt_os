...
**1-2MB:** 内存管理数据,进程管理数据
	...
	mm_v1方式：位示图的方式管理page
		...
		位示图1: 1MB + 132KB ~ 1MB + 136KB	使用4K的位示图来管理128MB的物理/内核内存，内核和物理地址一一对应(和Linux反过来，从0开始)。 虽然Linux的设计对编译程序虚拟地址时有好处，但是看得我很烦，所以就不用了。
		位示图2: 1MB + 132KB ~ 1MB + 136KB	使用4K的位示图来管理128MB的物理内存
